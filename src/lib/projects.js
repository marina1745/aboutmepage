...
const pictureMods = import.meta.glob("../assets/projectmedia/*.{png,jpg,jpeg,webp}?imagetools&w=720;1200;1600&format=avif;webp;jpg&as=picture", { eager: true });
// Raw images as strings (fallback option if needed)
const rawImageMods = import.meta.glob("../assets/projectmedia/*.{png,jpg,jpeg,webp}", { eager: true, import: "default" });
// Videos & audio -> plain strings
const videoMods = import.meta.glob("../assets/projectmedia/*.{mp4,webm,mov}", { eager: true, import: "default" });
const audioMods = import.meta.glob("../assets/projectmedia/*.{mp3,wav,ogg}", { eager: true, import: "default" });
// Small helper to build the glob key from filename
const K = (name) => `../assets/projectmedia/${name}`;
...
function pic(name) {
    ...
    if (pictureMods[keyWithQuery])
        return pictureMods[keyWithQuery];
    if (rawImageMods[K(name)])
        return rawImageMods[K(name)];
    // As a last resort, return a string path so nothing crashes
    return `/projectmedia/${name}`;
}
// Resolve to video/audio URLs
function vid(name) {
    return videoMods[K(name)] ?? `/projectmedia/${name}`;
}
function aud(name) {
    return audioMods[K(name)] ?? `/projectmedia/${name}`;
}
// Shorthand builders
const I = (name, alt) => ({ type: "image", src: pic(name), alt });
const V = (name, caption) => ({ type: "video", src: vid(name), caption });
const A = (name, caption) => ({ type: "audio", src: aud(name), caption });
...
export const projects = [
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "Campus Wars is a mobile serious game designed to promote social interaction and learning among university students. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "In *Sneaky Sneaky*, you’re a mischievous little mouse on a daring mission: steal the cheese without getting caught! " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "*Tempora Facta Casa* is a collaborative project with architecture students, created to explore the poetic side of architecture — and the quiet beauty of wood as it changes over time.\n\n" +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "Kingdom of Colors is a tile-matching puzzle game where players connect sides of matching colors to earn points. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "Escape Ancient China VR is a serious game developed in Unity for the Varjo XR-3 headset. The player, a university student who accidentally time-travels to ancient China, must escape three rooms representing different dynasties—the Qin, Han, and Tang periods—by solving historically themed puzzles.\n\n" +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "\"Just Another Kaindorf Story\" is one of my first game projects — a 2D puzzle adventure entirely developed in Unity, featuring hand-drawn pixel art created by myself. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "non-game",
        body: "A collection of physics simulations implemented entirely in C++, focusing on realistic behavior for potential game integration. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "non-game",
        body: "A full audio recreation of the official *Anthem* game trailer, with every sound effect and ambient cue recorded, performed, and edited by myself. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "non-game",
        body: "This project is a custom-built miniature relational database written entirely in C++, designed to deepen my understanding of database internals and query execution. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "non-game",
        body: "This piece represents my first attempt at creating original background music for games. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "academic",
        body: "This project is part of a research-focused virtual reality experiment developed at the Technical University of Munich. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "academic",
        body: "The 'Patient Information System' was developed as part of my diploma thesis at the Higher Technical College of Kaindorf in collaboration with BOOM Software AG. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "academic",
        body: "My Bachelor's thesis focused on developing a formalized model for representing and partitioning game worlds through hierarchical graph structures. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "non-game",
        body: "React Debate Trainer is a full-stack web application built as an exercise to integrate the OpenAI API into a custom React app. " +
    ...
    // -------------------------------------------------------------------------------
    {
        ...
        category: "game",
        body: "RTS Demo in Unreal is a small real-time strategy prototype I created to get hands-on experience with Unreal Engine’s gameplay framework and C++ workflow. " +

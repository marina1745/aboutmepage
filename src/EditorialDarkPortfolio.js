...
function Hero() {
    ...
    return (_jsxs("section", { id: "top", className: "relative overflow-hidden", children: [_jsxs("div", { className: "relative mx-auto grid max-w-6xl grid-cols-1 items-end gap-10 px-5 pb-24 pt-20 md:grid-cols-12 md:pt-28", children: [_jsx(motion.h1, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.7 }, className: "col-span-7 font-serif text-5xl leading-[1.05] tracking-tight md:text-7xl", children: "Games Engineer, Visual Computation & XR Researcher, Pretty Chill Girl" }), _jsxs(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.7, delay: 0.1 }, className: "col-span-5 md:pl-8", children: [_jsx("p", { className: "max-w-xl text-zinc-300 md:text-lg", children: "I work at the intersection of graphics, perception, and interaction - exploring how humans see, move, and connect in virtual worlds. My background in games engineering meets my curiosity for research, and together they keep me chasing beauty in systems, whether it\u2019s light in a shader or data in an eye tracker." }), _jsxs("div", { className: "mt-6 flex flex-wrap gap-3", children: [_jsxs("span", { className: "rounded-2xl border border-white/20 bg-white text-black px-5 py-3 text-sm font-medium \r\n                            hover:bg-zinc-200 inline-flex items-center transition-colors", children: [makeLink("work", "See Selected Work"), _jsx(ArrowUpRight, { className: "ml-1 inline h-4 w-4" })] }), _jsx(Link, { to: "/personal", className: "rounded-2xl border border-white/20 px-5 py-3 text-sm text-zinc-200 hover:border-white/40 inline-flex", children: "Meet me" })] })] })] }), _jsx("div", { className: "w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw]", children: _jsx(DeferredCarousel, { images: pics }) })] }));
}
